...
# 3) Creating a security group to allow inbound access to ports 22 (ssh) and 8080 (catering-service)
#    of EC2 instance
# 4) Creating an EC2 instance
# 5) Deploying and running Docker image vinodjagwani/catering-service
#    on EC2 instance to provision the catering-service
...
# Allow access to port 8080 for web app

aws ec2 authorize-security-group-ingress --group-name $SEC_GROUP_NAME --protocol tcp --port 8080 --cidr 0.0.0.0/0 > ${SEC_GROUP_NAME}.app.out 2>&1
if [ $? -ne 0 ]; then
...
docker pull ${AWS_ACCOUNT_ID}.dkr.ecr.us-east-1.amazonaws.com/vinodjagwani/catering-service:latest

docker run \
  -p 8080:8080 \
  ${AWS_ACCOUNT_ID}.dkr.ecr.us-east-1.amazonaws.com/vinodjagwani/catering-service:latest

EOF

echo
echo "INFO: Point your web browser to http://${HNAME}:8080 to verify"
echo
